...
def main() -> None:
    ...
    print()
    print("=" * w)
    print("WHAT THE MODEL ACTUALLY DOES AT A HUMAN-CALIBRATED G_pn")
    print("=" * w)
    print("  Measured 2026-08-25, reproduce with: julia --project=. bench/gpn_sweep.jl")
    print("  G_pn overridden per run; the model is built and simplified once.")
    print()
    print("    G_pn    shift (mmHg)   V_ecf range (L)    excretion/intake")
    print("    20.0        4.9342     14.174 - 14.560    1.0000  <- incumbent")
    print("    44.0        2.2830     14.381 - 14.560    1.0000  <- He 2002")
    print("    51.0        1.9737     14.405 - 14.560    1.0000  <- He 2013")
    print("    59.0        1.7091     14.426 - 14.560    1.0000  <- Cutler 1997")
    print("   188.0        0.5406     14.518 - 14.560    1.0000  <- Graudal 2019")
    print()
    print("  1. THE LOOP CLOSES EXACTLY AT EVERY VALUE. Excretion equals intake to")
    print("     four decimals at all three levels for all five slopes. There is no")
    print("     stability cost to the correction.")
    print()
    print("  2. V_ecf GETS TIGHTER, NOT LOOSER. The worry was that a large G_pn change")
    print("     might drive V_ecf toward the 10 L floor, as the G_vr sweep once did.")
    print("     It does the opposite - a steeper natriuretic response needs LESS volume")
    print("     excursion to excrete the same load. 14.43-14.56 L at G_pn=59 against")
    print("     14.17-14.56 at 20. The concern was misplaced.")
    print()
    print("  3. THE PREDICTED SHIFTS REPRODUCE THE HUMAN NUMBERS. G_pn=44 gives 2.283")
    print("     mmHg against He 2002's 2.30 per 100 mmol; 51 gives 1.974 against He")
    print("     2013's 1.96; 59 gives 1.709 against Cutler's 1.70. That is expected -")
    print("     G_pn was derived as 100/slope - but it confirms the model's")
    print("     dMAP ~ d(intake)/G_pn algebra holds numerically at these values, which")
    print("     was an assumption and is now a measurement.")
    print()
    print("  4. AND ONE THING WORTH FLAGGING. At Graudal's G_pn = 188 the shift is")
    print("     0.5406 mmHg, against the min_map_shift = 0.5 threshold in")
    print("     check_pressure_natriuresis. If the Graudal end of the controversy is")
    print("     right, the model's CENTRAL DEMONSTRATION becomes marginal - it passes")
    print("     its own falsifiable test by 8%. At the chronic-protocol values it")
    print("     passes comfortably. The choice between the two meta-analytic camps is")
    print("     therefore not only about a parameter; at one end it is about whether")
    print("     the effect this model exists to show is still visible.")
