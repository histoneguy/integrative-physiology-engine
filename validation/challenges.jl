...
println()
println("     THIS SAID 'THESE TWO FAILURES' ON 2026-09-07, 'ONE' ON 2026-09-08, AND")
println("     THERE ARE NOW NONE. Read that as a LOSS OF INFORMATION, not a result.")
println("       urine   770.976 -> 754.800 (ADR 0022) -> 738.134 mL   band 380-750")
println("       sodium  129.098 -> 126.478 (ADR 0022) -> 123.756 mmol band  63-127")
println("     Two independently sourced reflex corrections buffered the pressure rise,")
println("     so less sodium leaves by pressure natriuresis. Neither was fitted here:")
println("     the chronotropic gain is Laitinen 1998 and the vasomotor gain is")
println("     Yamasaki's own human measurement. RN.MD.RENIN_GAIN was NOT re-solved.")
println()
println("     WHAT WAS LOST. These two endpoints were the only quantitative BOUND on")
println("     the macula densa arm - ADR 0021 amendment A6 measured that it can carry")
println("     a chronic renin ratio of about 2.57 before the acute limb leaves its")
println("     band, against a ledger 2.73. Inside the band, they bound nothing, so")
println("     ADR 0021's prediction that renal sympathetic traffic lowers g_md is no")
println("     longer testable against Lobo. A GREEN HARNESS IS LESS INFORMATIVE HERE")
println("     THAN THE RED ONE WAS.")
